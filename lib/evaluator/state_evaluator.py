...
from lib.evaluator.abstract_evaluator import AbstractEvaluator


class StateEvaluator(AbstractEvaluator):
    ...
    def _check_config(self):
        ...
        for metric in self.metrics:
            if metric not in self.allowed_metrics:
                raise ValueError('the metric {} is not allowed in StateEvaluator'.format(str(metric)))
    ...
    def save_result(self, save_path, filename=None):
        ...
        if 'csv' in self.save_modes:
            ...
            self._logger.info("\n" + str(dataframe))
            self._logger.info('\n' + str(dataframe[['MAE','MAPE','MSE','RMSE']].mean()))
        return dataframe
