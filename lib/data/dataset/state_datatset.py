...
from lib.data.dataset import AbstractDataset
from lib.data.utils import generate_dataloader
from lib.utils import StandardScaler, NormalScaler, NoneScaler, \
    MinMax01Scaler, MinMax11Scaler, LogScaler, ensure_dir


class StateDataset(AbstractDataset):
    """
    交通状态预测数据集的基类。
    ...
    def __init__(self, config):
        ...
        self.cache_file_name = os.path.join('./lib/cache/dataset_cache/',
                                            'state_{}.npz'.format(self.parameters_str))
        self.cache_file_folder = './lib/cache/dataset_cache/'
    ...
    def _load_dyna_3d(self, filename):
        ...
        dynafile['lat'] = dynafile['dynaimc_coordinates'].map(lambda x: float(x.split(",")[0][1:]))
        dynafile['lon'] = dynafile['dynaimc_coordinates'].map(lambda x: float(x.split(",")[1][:-1]))
        ...
        if self.data_col != '':  # 根据指定的列加载数据集
            ...
            # 增加地理位置信息 Todo
            data_col.insert(2, 'lat')
            data_col.insert(3, 'lon')
