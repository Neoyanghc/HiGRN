...
class MutiClusterGCNCell(nn.Module):
    def __init__(self, dim_in, dim_out, cheb_k, embed_dim, layer):
        ...
        self.attlinear = nn.Linear(278 * dim_out, 1)
...
class HiGRU(AbstractStateModel):
    ...
    def forward(self, batch):
        ...
        output, hidden_states, a_loss = self.encoder(source, init_state, self.node_embeddings, graph)  # B, T, N, hidden
        ...
        # inputs = [output_cnn, output_2]
        # output = inputs.reshape(output_cnn.shape[0], output_cnn[1],output_cnn[2], -1)
        # out = self.attlinear(output)  # (batch, graph, 1)
        # weight = F.softmax(out, dim=1)
        #
        # outputs = (x * weight).sum(dim=1).reshape(b, n, f)
