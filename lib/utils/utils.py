...
def get_executor(config, model):
    ...
    """
    try:
        return getattr(importlib.import_module('lib.executor'),
...
def get_model(config, data_feature):
    ...
    if config['task'] == 'traj_loc_pred':
        try:
            return getattr(importlib.import_module('lib.model.trajectory_loc_prediction'),
                           config['model'])(config, data_feature)
        except AttributeError:
            raise AttributeError('model is not found')
    elif config['task'] == 'state_pred':
        try:
            return getattr(importlib.import_module('lib.model.flow_prediction'),
                           config['model'])(config, data_feature)
        except AttributeError:
            try:
                return getattr(importlib.import_module('lib.model.speed_prediction'),
                               config['model'])(config, data_feature)
            except AttributeError:
                try:
                    return getattr(importlib.import_module('lib.model.demand_prediction'),
                                   config['model'])(config, data_feature)
                except AttributeError:
                    try:
                        return getattr(importlib.import_module('lib.model.od_prediction'),
                                       config['model'])(config, data_feature)
                    except AttributeError:
                        try:
                            return getattr(importlib.import_module('lib.model.accident_prediction'),
                                           config['model'])(config, data_feature)
                        except AttributeError:
                            raise AttributeError('model is not found')
    elif config['task'] == 'map_matching':
        try:
            return getattr(importlib.import_module('lib.model.map_matching'),
                           config['model'])(config, data_feature)
        except AttributeError:
            raise AttributeError('model is not found')
    elif config['task'] == 'road_representation':
        try:
            return getattr(importlib.import_module('lib.model.road_representation'),
                           config['model'])(config, data_feature)
        except AttributeError:
            raise AttributeError('model is not found')
    elif config['task'] == 'eta':
        try:
            return getattr(importlib.import_module('lib.model.eta'),
                           config['model'])(config, data_feature)
        except AttributeError:
            raise AttributeError('model is not found')
    else:
...
def get_evaluator(config):
    ...
    """
    try:
        return getattr(importlib.import_module('lib.evaluator'),
